...
$.widget( "ui.menuex", $.ui.menu, {
	...
	_open: function( submenu ) {
		...
		case "down":
			var position = $.extend( {
				of: this.active
			}, { my: "left top", at: "left+0 bottom+1"} );
			break;
		case "right":
			var position = $.extend( {
				of: this.active
	...
	_create: function() {           
	  var _this = this;
		...
		if (this.options.validateOnNext == 0){	
			$('[name="wiznext_btn"]').off('click.wiz').on('click.wiz',function(e){
					_this.next();
				});
		} else {
			$('[name="wiznext_btn"]').off('click.wiz').on('click.wiz',function(e){
		...
		$('[name="wizprevious_btn"]').off('click.wiz').on('click.wiz',function(e){
			_this.previous();
			});
...
ntdialog.prototype.save = function (event){
	...
	if (ntdLocalStorage){
		...
		var form = $("#"+this.dlg[this.level]+"_frm")
		var formdata = form.serialize() + '&pressedButton=save_btn&_popup_=1&_ajax_=1&_grandparent_=' + this.grandparent[this.level] + '&_buttontext_=' + $(event.target).text()
		$.ajax({
           type: "POST",
...
//		28 may 2020. The code below uses the jquery.for.js extension. Seems to be broken in Chrome 83. https://github.com/jquery-form/form
		// var options = {
			// url: id,
			// dataType: 'xml',
			// success:    function(data) { 
...
var ntdLocalStorage=0;;var NetTalkVersion=11.51
// ---------------------------------------------
...
// ---------------------------------------------
function ntAlert(m,t,timer,oktext){
	if(!oktext){oktext = 'OK'}
	setTimeout(function() {
        var a = jQuery(":focus").attr('id');
		$("#message_alert").remove();
