...
//------------------------------------------------------
	locate: function(zoom){	
		...
		navigator.geolocation.getCurrentPosition(
			function(pos){ // success 
				_this.updateMarker('_home_',pos.coords.latitude,pos.coords.longitude) 
...
//------------------------------------------------------
	updateMarker: function(bidv,lat,lng,options) {
		$(this.options.markers).each(function(i,m){
				if (m[bidv]){
					m[bidv].setLatLng([lat,lng]);
					if (options){
						if (options.icon){	
							m[bidv].setIcon(options.icon);	
						}
						if (options.opacity){	
							m[bidv].setOpacity(options.opacity);	
						}					
						if (options.title){	
							$(m[bidv]._icon).attr('title',options.title);
						}					
					}					
					return // found a top-level marker
				}
			})	
...
//------------------------------------------------------
	mandatoryTileOptions: function(p,options) {           	
		switch(p){		
			...
			case 55: //Net:HERE:TerrainDay           
			case 56: //Net:HERE:TrafficDay           
				options.attribution='Map &copy; <a href="https://developer.here.com">HERE</a>, Data &copy; NAVTEQ 2012';
				options.subdomains="1234";
				break;
			default:
				options.attribution='Tiles Courtesy of <a href="http://www.mapquest.com/">MapQuest</a> &mdash; Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>';
...
//------------------------------------------------------  
	getURL: function(p) {
		...
		switch(p){		
			...
			case 51: //Net:HERE:NormalDay     
				if (this.options.tileOptions.apiKey){
					h='https://'
					return(h+'{s}.base.maps.ls.hereapi.com/maptile/2.1/maptile/newest/normal.day/{z}/{x}/{y}/256/png?apiKey={apiKey}')
				} else {
			...
			case 52: //Net:HERE:NormalGreyDay
				if (this.options.tileOptions.apiKey){
					h='https://'
					return(h+'{s}.base.maps.ls.hereapi.com/maptile/2.1/maptile/newest/normal.day.grey/{z}/{x}/{y}/256/png?apiKey={apiKey}')
				} else {
			...
			case 53: //Net:HERE:SatelliteNoLabelsDay 
				if (this.options.tileOptions.apiKey){
					h='https://'
					return(h+'{s}.aerial.maps.ls.hereapi.com/maptile/2.1/maptile/newest/satellite.day/{z}/{x}/{y}/256/png?apiKey={apiKey}')
				} else {
			...
			case 54: //Net:HERE:SatelliteYesLabelsDay
				if (this.options.tileOptions.apiKey){
					h='https://'
					return(h+'{s}.aerial.maps.ls.hereapi.com/maptile/2.1/maptile/newest/hybrid.day/{z}/{x}/{y}/256/png?apiKey={apiKey}')	
				} else {
					return(h+'{s}.aerial.maps.ls.hereapi.com/maptile/2.1/maptile/newest/hybrid.day/{z}/{x}/{y}/256/png?app_id={devID}&app_code={appID}')	
				}
			case 55: //Net:HERE:TerrainDay           
				if (this.options.tileOptions.apiKey){
					h='https://'
					return(h+'{s}.aerial.maps.ls.hereapi.com/maptile/2.1/maptile/newest/terrain.day/{z}/{x}/{y}/256/png?apiKey={apiKey}')
				} else {
			...
			case 56: //Net:HERE:TrafficDay           
				if (this.options.tileOptions.apiKey){
					h='https://'
					return(h+'{s}.traffic.maps.ls.hereapi.com/maptile/2.1/traffictile/newest/terrain.day/{z}/{x}/{y}/256/png?apiKey={apiKey}')
				} else {
