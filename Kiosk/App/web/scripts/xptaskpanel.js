	...
	*/	
	...
	function initDhtmlgoodies_xpPane(paneId,panelTitles,panelDisplayed,cookieArray,panelClassName,contentClassName)
	{
	  if (panelClassName=='undefined'){
	    panelClassName = 'dhtmlgoodies_panel'
	  }
	  if (contentClassName=='undefined'){
	    contentClassName = 'panelContent'
	  }
	  
		dhtmlgoodies_xpPane = document.getElementById(paneId); // document.getElementById('dhtmlgoodies_xpPane'); //bj
		
		//if (document.body.clientHeight > 50){
        //  dhtmlgoodies_xpPane.style.height=document.body.clientHeight-50 + 'px';
        //}
		var divs = dhtmlgoodies_xpPane.getElementsByTagName('DIV');
		dhtmlgoodies_paneIndex=0;
		cookieNames = cookieArray;
		for(var no=0;no<divs.length;no++){
			if(divs[no].className==panelClassName){
				...
				outerContentDiv.className = contentClassName;
				...
				img.src = '/styles/images/arrow_up.gif';				
				...
				if(!panelDisplayed[dhtmlgoodies_paneIndex]){
					...
					img.src = '/styles/images/arrow_down.gif';
				}
